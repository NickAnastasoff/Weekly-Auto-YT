...
def text_box(text="text", fontsize=40, opac=.5, y=0, width=0, height=0,
...
def text_clip_from_choices(choices):
    ...
    for i, choice in enumerate(choices):
        text_clip = text_box(
            choice[0], fontsize=80, opac=0.0, y=(i * 40) + 1.5,
...
def resized_image_clip(image, duration=5, start_time=0, h=384, y=0):
    ...
    image_clip = image_clip.set_pos(
        lambda _: ((video_width - image_clip.w) / 2, y)
    )
    return image_clip
...
def main():
    ...
    for iteration in range(iterations):
        ...
        longest_words = [sorted(choice[0].split(), key=len, reverse=True) for choice in choices]
        get_best_images(longest_words)
        image_clips = resized_image_clip("Week2/images/0.jpg", y=384*1.5), resized_image_clip("Week2/images/1.jpg", y=384*3.5)
